...
func makeQuantiles(basename string, m *dto.Metric) []*dataobj.MetricValue {
	...
	metrics = append(metrics, model.NewSubtractMetric(countName, m.GetSummary().SampleCount, now, tags))

	sumName := fmt.Sprintf("%s_sum", basename)
	metrics = append(metrics, model.NewSubtractMetric(sumName, m.GetSummary().SampleSum, now, tags))
...
func makeBuckets(basename string, m *dto.Metric) []*dataobj.MetricValue {
	...
	countName := fmt.Sprintf("%s_count", basename)
	metrics = append(metrics, model.NewSubtractMetric(countName, m.GetHistogram().SampleCount, now, tags))

	sumName := fmt.Sprintf("%s_sum", basename)
	metrics = append(metrics, model.NewSubtractMetric(sumName, m.GetHistogram().SampleSum, now, tags))
...
func makeCommon(metricName string, m *dto.Metric) []*dataobj.MetricValue {
	...
	if m.Gauge != nil {
		...
		if !math.IsNaN(m.GetCounter().GetValue()) {
			...
			metrics = append(metrics, model.NewSubtractMetric(metricName, val, now, tags))
		}
